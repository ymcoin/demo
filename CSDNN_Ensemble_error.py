...
def test_SdA(p_dict,ens_param, logger):
    ...
    for mm in range(num_of_fold):
        ...
        #                p_dict['pretrain_batchsize'], p_dict['cl'], p_dict['beta'], p_dict['exp_name']))
    ...
    # logger.info('Main Model: Layer:%s, CostVec:%s, fineLR:%s, preLR:%s, fineBatch:%s, preBatch:%s, cl:%s, beta:%s, Name:%s'
    #             % (p_dict['h'], p_dict['cost_vec'], p_dict['finetune_lr'], p_dict['pretrain_lr'], p_dict['batch_size'],
    #                p_dict['pretrain_batchsize'], p_dict['cl'], p_dict['beta'], p_dict['exp_name']))
    ...
    # main_summary_logger.info(
    #     'Main Model: Layer:%s, CostVec:%s, fineLR:%s, preLR:%s, fineBatch:%s, preBatch:%s, cl:%s, beta:%s, Name:%s'
    #     % (p_dict['h'], p_dict['cost_vec'], p_dict['finetune_lr'], p_dict['pretrain_lr'], p_dict['batch_size'],
    #        p_dict['pretrain_batchsize'], p_dict['cl'], p_dict['beta'], p_dict['exp_name']))
...
#================
    e = {'h': [100, 100], 'training_epochs': 100, 'pretraining_epochs': 200, 'finetune_lr': 0.01, 'beta': 1,
         'cl': [0.1, 0.1], 'cost_vec': [1, 3], 'batch_size': 1, 'pretrain_lr': 0.001, 'exp_name': 'main_model',
         'pretrain_batchsize': 10, 'reg_coef': 0.005, 'num_subset': 10, 'drop_ps': [.5,.5,.5,.5]}

    experiment_list.append(dict(e))
    ens_param_list.append(
        {'h': [100], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
         'cl': [0.1, 0.1, 0.1], 'cost_vec': [1, 1], 'batch_size': 20, 'pretrain_lr': 0.001,
         'pretrain_batchsize': 10, 'reg_coef': 0.005, 'drop_ps': [.5,.5,.5,.5], 'exp_name': None})
    experiment_list.append(dict(e))
    ens_param_list.append(
        {'h': [100], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.01, 'beta': 1,
         'cl': [0.1, 0.1, 0.1], 'cost_vec': [1, 1], 'batch_size': 20, 'pretrain_lr': 0.001,
         'pretrain_batchsize': 10, 'reg_coef': 0.005, 'drop_ps': [1,1,1,1], 'exp_name': None})
    experiment_list.append(dict(e))
    ens_param_list.append(
        {'h': [90], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.001, 'beta': 1,
         'cl': [0.1, 0.1, 0.1], 'cost_vec': [1, 1], 'batch_size': 20, 'pretrain_lr': 0.001,
         'pretrain_batchsize': 10, 'reg_coef': 0.005, 'drop_ps': [1,1,1,1], 'exp_name': None})
    experiment_list.append(dict(e))
    ens_param_list.append(
        {'h': [80], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.001, 'beta': 1,
         'cl': [0.1, 0.1, 0.1], 'cost_vec': [1, 1], 'batch_size': 20, 'pretrain_lr': 0.001,
         'pretrain_batchsize': 10, 'reg_coef': 0.005, 'drop_ps': [1,1,1,1], 'exp_name': None})
    experiment_list.append(dict(e))
    ens_param_list.append(
        {'h': [60], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.001, 'beta': 1,
         'cl': [0.1, 0.1, 0.1], 'cost_vec': [1, 1], 'batch_size': 20, 'pretrain_lr': 0.001,
         'pretrain_batchsize': 10, 'reg_coef': 0.005, 'drop_ps': [1,1,1,1], 'exp_name': None})
    experiment_list.append(dict(e))
    ens_param_list.append(
        {'h': [120], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.001, 'beta': 1,
         'cl': [0.1, 0.1, 0.1], 'cost_vec': [1, 1], 'batch_size': 20, 'pretrain_lr': 0.001,
         'pretrain_batchsize': 10, 'reg_coef': 0.005, 'drop_ps': [1,1,1,1], 'exp_name': None})

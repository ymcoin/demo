...
def test_SdA(p_dict,ens_param, logger):
    ...
    vote_result = []
    num_of_fold = 10
    if (data_dir == 'data/churn' or data_dir == 'data/kdd98' or data_dir == 'data/portugese'):
        num_of_fold = 1
    num_subset = p_dict['num_subset']
    last_model_path = 'log/' + data_dir + '/last_model_'
    # check if same model existed
    p_dict_backup = dict(p_dict)
    is_model_exist = exist_model(p_dict_backup)
    if (is_model_exist):
        logger.info('load last model')
    else:
        logger.info('no last model')
    ...
    for mm in range(num_of_fold):
        ...
        if (is_model_exist == False):
            ...
            save_file('log/' + data_dir + '/last_model_' + str(mm + 1), load_model(main_model_path))
        else:
            logger.info("Trail K=%d" % (mm + 1))
            logger.info('load last model')
            main_model_path = last_model_path + str(mm + 1)
            main_train_result, main_test_result = compute_result_from_pretrain_model(main_model_path,
                                                                                     p_dict['train_path'],
                                                                                     p_dict['test_path'])

        # Prepare for ensemble learning
        data_list = prepare_data_ensemble_learning(train_path=p_dict['train_path'], test_path=p_dict['test_path'],
                                                   model_path=main_model_path, num_subset=num_subset)
        ens = Ensemble(data_list, ens_param, 'csdnn_dynamic_cost')
...
# Portugese
#     main_summary_logger.info("Cost_dyn AUC, delay 5 sub 10 Repeat 10 times")
#     e = {'h': [100], 'training_epochs': 100, 'pretraining_epochs': 300, 'finetune_lr': 0.01, 'beta': 1,
#          'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [1, 1], 'batch_size': 1, 'pretrain_lr': 0.001,
#          'exp_name': 'main_model',
#          'pretrain_batchsize': 20, 'reg_coef': None, 'num_subset': 10, 'drop_ps': [.5, .5, .5, .5]}
#
#     experiment_list.append(dict(e))
#     ens_param_list.append(
#         {'h': [200], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
#          'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.01, 0.05, 0.005], 'batch_size': 100, 'pretrain_lr': 0.001,
#          'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
# Taiwan
    main_summary_logger.info("Cost_dyn AUC, delay 3 sub 10 Repeat 10 times")
    e = {'h': [200,200], 'training_epochs': 100, 'pretraining_epochs': 300, 'finetune_lr': 0.01, 'beta': 1,
             'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [1, 2.7], 'batch_size': 1, 'pretrain_lr': 0.001,
             'exp_name': 'main_model',
             'pretrain_batchsize': 20, 'reg_coef': None, 'num_subset': 10, 'drop_ps': [.5, .5, .5, .5]}

    experiment_list.append(dict(e))
    ens_param_list.append(
            {'h': [150], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
             'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.5,3,0.8], 'batch_size': 100, 'pretrain_lr': 0.001,
             'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})


    experiment_list = [dict(experiment_list[0]) for i in range(REPEAT_TIME)]
    ens_param_list = [dict(ens_param_list[0]) for i in range(REPEAT_TIME)]
...
#================
    ...
    main_summary_logger.info("Cost_dyn AUC, delay 3 sub 50")
    ...
    experiment_list.append(dict(e))
    ens_param_list.append(
        {'h': [120], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
         'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.1, 0.5, 0.05], 'batch_size': 100, 'pretrain_lr': 0.001,
         'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    ...
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.1, 0.5, 0.05], 'batch_size': 100, 'pretrain_lr': 0.001,
    #      'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    # e = {'h': [60], 'training_epochs': 100, 'pretraining_epochs': 300, 'finetune_lr': 0.01, 'beta': 1,
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [1, 1], 'batch_size': 1, 'pretrain_lr': 0.001,
    #      'exp_name': 'main_model',
    #      'pretrain_batchsize': 20, 'reg_coef': None, 'num_subset': 10, 'drop_ps': [.5, .5, .5, .5]}
    #
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    #     {'h': [60], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.1, 0.5, 0.05], 'batch_size': 100, 'pretrain_lr': 0.001,
    #      'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    ...
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    #     {'h': [200, 200], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.5, 2.5, 0.3], 'batch_size': 100, 'pretrain_lr': 0.001,
    #      'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    #
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    #     {'h': [200, 200], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.5, 2.5, 0.5], 'batch_size': 100, 'pretrain_lr': 0.001,
    #      'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    #
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    #     {'h': [200, 200], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.5, 2.5, 0.7], 'batch_size': 100, 'pretrain_lr': 0.001,
    #      'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    #
    ...
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    #     {'h': [150, 150], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.01, 0.8, 0.005], 'batch_size': 100, 'pretrain_lr': 0.001,
    #      'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    #
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    ...
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    #     {'h': [250, 250], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.01, 0.8, 0.005], 'batch_size': 100, 'pretrain_lr': 0.001,
    #      'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    #
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    #     {'h': [150, 150], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.01, 0.5, 0.05], 'batch_size': 100, 'pretrain_lr': 0.001,
    #      'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    #
    # experiment_list.append(dict(e))
    # ens_param_list.append(
    ...
    # ens_param_list.append(
    #     {'h': [150, 150], 'training_epochs': 200, 'pretraining_epochs': 0, 'finetune_lr': 0.005, 'beta': 1,
    #      'cl': [0.1, 0.1, 0.1, .1], 'cost_vec': [0.01, 0.8, 0.05], 'batch_size': 100, 'pretrain_lr': 0.001,
    #      'pretrain_batchsize': 10, 'reg_coef': None, 'drop_ps': [.5, .5, .5, .5], 'exp_name': None})
    #
    # experiment_list.append(dict(e))
    # ens_param_list.append(
